...
window.moveBranchRight = async info => {
  ...
  const num = numChildren(branches, index);

  for (let i = 0; i < num + 1; ++i) if (branches[index+1].level >= 5) return sendMessage(socket, 'Cannot indent branches beyond five levels');

  for (let i = 0; i < num + 1; ++i) ++branches[index+i].level;
...
window.moveBranchLeft = async info => {
  ...
  const curChildren = numChildren(branches, index);
  for (let i = index; i <= index + curChildren; ++i) --branches[i].level;
  ...
  if  (prevSibling) {
    ...
    const removed = branches.splice(index, curChildren + 1);
  
    // Insert the removed branches after the prevSibling and its children
    const prevSiblingChildren = numChildren(branches, prevSibling);
    insertIntoArray(branches, prevSibling + prevSiblingChildren + 1, removed);
  }
